...
import { getPublicHotelBySlug } from "@/lib/aether/booking-fns";
...
export const Route = createFileRoute("/$hotelSlug")({
  ...
  loader: async ({ params }) => getPublicHotelBySlug({ data: { publicSlug: params.hotelSlug } }),
  component: PublicHotelSlug,
});

function PublicHotelSlug() {
  const result = Route.useLoaderData();
  if (!result.ok) return <UnknownHotel message={result.message || touristMessage("hotel_not_found")} />;
  return (
    <GuestBook
...
function UnknownHotel({ message }: { message: string }) {
  return (
    <main className="flex min-h-dvh flex-col bg-canvas text-ink">
      <div className="flex justify-end px-5 py-4"><ThemeToggle /></div>
      <section className="flex flex-1 flex-col items-center justify-center px-6 pb-16 text-center">
        ...
        <p className="mt-4 max-w-sm text-base leading-relaxed text-muted">Please ask reception for the correct booking link.</p>
      </section>
    </main>
  );
}
