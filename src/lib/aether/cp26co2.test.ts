...
test("CP26C-O2 source: protected /owner shell, no commerce mutation, no Domain B revenue", () => {
  ...
  assert.equal((fns.match(/createServerFn\(/g) ?? []).length, 6);
  assert.equal((fns.match(/await requirePlatformOwner/g) ?? []).length, 6);
  ...
  assert.match(plans, /CP26C-O3|pending catalogue/i);
  ...
  assert.equal(pkg.scripts["db:migrate:0025"], undefined);
  ...
  assert.doesNotMatch(preflight, /0025_cp26co2_platform_owners\.sql/);
